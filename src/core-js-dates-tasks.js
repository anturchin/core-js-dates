...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const days = Math.floor(differenceMs / (1000 * 60 * 60 * 24)) + 1;
...
function formatDate(date) {
  ...
  const hours = formattedDate.getUTCHours().toString();
...
function getWeekNumberByDate(date) {
  ...
  if (dayOfWeek !== 1) {
    startOfYear.setDate(1 - ((dayOfWeek + 6) % 7));
  }
...
function getNextFridayThe13th(date) {
  for (let month = date.getMonth(); month < 12; month += 1) {
    const nextFriday13 = new Date(date.getFullYear(), month, 13);
    if (nextFriday13.getDay() === 5) {
      if (nextFriday13 > date) {
        return nextFriday13;
      }
    }
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startComponents = period.start.split('-').map(Number);
  ...
  const endComponents = period.end.split('-').map(Number);
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatDateToString(currentDate));
